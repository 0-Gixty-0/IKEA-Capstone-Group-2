  ...
  try {
    ...
    if (session) {
      ...
      if (postId && roles) {
        ...
        const usersWithPostInReadingListIds = new Set(
          usersWithPostInReadingList.map((user) => user.id),
        );

        const readUsers = usersWithMatchingRoles.filter(
          (user) => !usersWithPostInReadingListIds.has(user.id),
        );
    ...
    return NextResponse.json(
      { error: "Something went wrong when fetching the reading list!" },
      { status: 500 },
    );
  }
}
